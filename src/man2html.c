 ...
 *
 * man2html-WIN32-1.5
 * Modified for available on WIN32 environment. (with -DWIN32 and -DMSDOS)
 * Added support for man pages written in Japanese. (with -DUSE_JAPANESE)
...
#ifdef  WIN32
#ifdef  _MSC_VER	/* for MS-C, Visual C++ */
#define popen       _popen
#define pclose      _pclose
#endif
#ifdef  __LCC__		/* for lcc-win32 */
int     pclose( FILE * );
#endif
int	strncmpi( char *, char *, size_t );
#define	strncasecmp	strncmpi
#endif

#define	iskanjiStart(c) (((*c) == 27) && ((*(c+1)) == '$') && ((*(c+2)) == 'B'))
#define	iskanjiEnd(c)   (((*c) == 27) && ((*(c+1)) == '(') && ((*(c+2)) == 'J'))
...
#else
// #define CGIBASE "http://localhost/man2html/man2html.cgi"
#define CGIBASE "http://localhost/~tsujimura543/man2html/man2html.cgi"
#endif
#endif
...
#else
#define TIMEFORMAT "%H:%M:%S GMT, %B %d, %Y"
#endif
...
#ifdef	USE_JAPANESE
		    "/usr/share/man/japan/",
		    "/usr/share/jsmman/japan/",
		    "/usr/share/mnlsman/japan/",
		    "/usr/share/ucbman/japan/",
		    "/usr/share/xxman/japan/",
#endif
		    "/usr/share/man/",
		    "/usr/share/jsmman/",
		    "/usr/share/mnlsman/",
...
static char *scan_escape(char *c)
{
    ...
    case '0': out_html(change_to_font('R')); /* {@@} */ /* not break; */
    case ' ': h="&nbsp;";curpos++; break;
    case '|': h=""; break;
		...
		if (*h==' ' && (h[-1]=='\n' || usenbsp)) {
		    ...
		    intbuff[ibp++]=*h++;
                    do {
                        cnt++;
		        intbuff[ibp++]=*h;
	                if (((cnt % 2) == 0) && (ibp > (MED_STR_MAX - 20))) FLUSHIBP;
                        h++;
                    } while ( !iskanjiEnd( h ) );
                    if ( iskanjiEnd( h ) ) {
		        intbuff[ibp++]=*h++;
...
static void get_man_config( void )
{
#ifdef MAN_CONFIG
  FILE *config = NULL;
